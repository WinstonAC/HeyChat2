...
const Toast = ({ id, message, type, duration = 3000, onDismiss }) => {
    ...
    if (!message)
        return null; // Don\'t render if message is somehow empty
    return (<div className={`${baseClasses} ${typeClasses[type]} ${visibilityClasses} mb-2 max-w-sm w-full`} role="alert">
      <div className="flex justify-between items-center">
        <span>{message}</span>
        <button onClick={() => {
            setIsVisible(false);
            setTimeout(() => onDismiss(id), 300);
        }} className="ml-2 text-xl font-semibold leading-none hover:opacity-75" aria-label="Dismiss">
          &times;
        </button>
      </div>
    </div>);
