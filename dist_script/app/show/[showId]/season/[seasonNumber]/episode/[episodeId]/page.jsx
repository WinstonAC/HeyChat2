...
export default function EpisodeConversationPage() {
    ...
    const seasonNumberStr = Array.isArray(params.seasonNumber) ? params.seasonNumber[0] : params.seasonNumber;
    ...
    const [commentRefreshTrigger, setCommentRefreshTrigger] = useState(Date.now());
    ...
    return (<div className="container mx-auto px-4 py-8 max-w-3xl">
      {episodeDetails ? (<>
          ...
          <div className="text-sm text-gray-400 mb-1">
            ...
            {episodeDetails.air_date && (<span className="ml-2 pl-2 border-l border-gray-600">
                Aired: {formatDate(episodeDetails.air_date, { year: 'numeric', month: 'long', day: 'numeric' })}
              </span>)}
          </div>
          {episodeDetails.description && (<p className="text-gray-300 mt-2 mb-6 whitespace-pre-line">
      ...
      {pageError && episodeDetails && (<div className="my-4 p-3 bg-yellow-800 border border-yellow-600 rounded-md">
      ...
      <div className="mt-8">
        ...
        <CommentList showId={showId} seasonNumber={seasonNumber} episodeId={episodeId} refreshTrigger={commentRefreshTrigger}/>
      </div>

      <CommentInput showId={showId} seasonNumber={seasonNumber} episodeId={episodeId} author={hardcodedAuthor} onCommentPosted={handleCommentPosted}/>
    </div>);
}
