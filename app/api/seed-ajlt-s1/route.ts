...
async function fetchTmdbShowId(showName: string): Promise<number | null> {
  if (!TMDB_API_KEY) {
    console.warn('[API /api/seed-ajlt-s1] TMDB_API_KEY is not set. Cannot fetch show ID.');
    return null;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('[API /api/seed-ajlt-s1] TMDb search API error:', errorData);
      return null;
    }
    ...
    console.warn(`[API /api/seed-ajlt-s1] No show found for "${showName}" on TMDb.`);
    return null;
  } catch (error) {
    console.error(`[API /api/seed-ajlt-s1] Error fetching show ID for "${showName}":`, error);
    return null;
  }
}
...
async function fetchTmdbSeasonEpisodes(tmdbShowId: number, seasonNumber: number): Promise<any[]> {
  if (!TMDB_API_KEY) {
    console.warn('[API /api/seed-ajlt-s1] TMDB_API_KEY is not set. Cannot fetch season episodes.');
    return [];
  }
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json();
      console.error('[API /api/seed-ajlt-s1] TMDb season API error:', errorData);
      return [];
    }
    const data = await response.json();
    return data.episodes || [];
  } catch (error) {
    console.error(`[API /api/seed-ajlt-s1] Error fetching season ${seasonNumber} for TV ID ${tmdbShowId}:`, error);
    return [];
  }
}

export async function POST() {
  console.log('[API /api/seed-ajlt-s1] Received POST request to seed episodes.');

  if (!TMDB_API_KEY) {
    console.error('[API /api/seed-ajlt-s1] TMDB_API_KEY is not configured.');
  ...
  try {
    ...
    for (const episode of formattedEpisodes) {
      ...
      if (selectError && selectError.code !== 'PGRST116') { // PGRST116: Row to VARYING relation does not exist (means no record found)
        console.error(`[API /api/seed-ajlt-s1] Error checking for existing episode ${episode.episode_number}:`, selectError);
        // Decide if you want to continue or abort for this specific error
        continue; 
      }

      if (existingEpisode) {
        console.log(`[API /api/seed-ajlt-s1] Episode ${episode.episode_number} for show ${episode.show_id} already exists. Skipping.`);
        continue;
      }
      ...
      if (insertError) {
        console.error(`[API /api/seed-ajlt-s1] Error inserting episode ${episode.episode_number}:`, insertError);
        // Decide if you want to log this and continue, or stop the whole process
      } else {
        console.log(`[API /api/seed-ajlt-s1] Successfully inserted episode ${episode.episode_number} for show ${episode.show_id}.`);
        newEpisodesAdded++;
      }
    }
    ...
    console.error('[API /api/seed-ajlt-s1] General error during seeding process:', error);
    return NextResponse.json(
