...
// Define the expected structure for an episode, matching your Supabase schema
interface FormattedEpisode {
  show_id: string; // This will be a placeholder, replace with your actual DB foreign key
  episode_number: number;
  title: string;
  air_date: string | null;
  overview: string | null;
  still_path: string | null;
  // Optional: Add season_number or tmdb_episode_id if you want to store them
...
async function fetchShowId(showName: string): Promise<number | null> {
  if (!TMDB_API_KEY) {
...
async function fetchSeasonEpisodes(tvId: number, seasonNumber: number): Promise<any[]> {
  if (!TMDB_API_KEY) {
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      message: (error as Error).message || 'An unexpected error occurred. Using dummy data.',
      episodes: dummyEpisodes,
    }, { status: 500 });
  }
}
...
function getDummyEpisodesPlaceholder(): FormattedEpisode[] {
